...
_logger_cls = getattr(logging, "Logger", None)
if _logger_cls is not None and not hasattr(_logger_cls, "detailed"):
    _logger_cls.detailed = _logger_cls.debug
...
class InvalidValuesCheck(DiagnosticFunction):
    ...
    def diagnostic(self, net, **kwargs):
        ...
        valve_table = net["valve"] if hasattr(net, "valve") else None
        if valve_table is not None and not valve_table.empty and {"element", "et"}.issubset(valve_table.columns):
            pipe_index = net.pipe.index if hasattr(net, "pipe") else []
            for idx, row in valve_table.iterrows():
                value = row["element"]
                if row["et"] == "ju" and value not in net.junction.index:
                    result.setdefault("valve", []).append(
                        (idx, "element", value, "existing_junction for et='ju'")
                    )
                elif row["et"] == "pi" and value not in pipe_index:
                    result.setdefault("valve", []).append(
                        (idx, "element", value, "existing_pipe for et='pi'")
                    )
                elif row["et"] not in ("ju", "pi"):
                    result.setdefault("valve", []).append(
                        (idx, "et", row["et"], "one of 'ju' or 'pi'")
                    )
...
class MissingBranchJunctionsCheck(DiagnosticFunction):
    ...
    def diagnostic(self, net, **kwargs):
        ...
        if hasattr(net, "valve") and not net.valve.empty and {"junction", "element", "et"}.issubset(net.valve.columns):
            missing_j = np.setdiff1d(net.valve.junction, net.junction.index)
            missing_element_j = np.setdiff1d(
                net.valve.loc[net.valve.et == "ju", "element"],
                net.junction.index,
            )
            if len(missing_j) or len(missing_element_j):
                result["valve"] = {
                    "missing_from_junctions": missing_j,
                    "missing_to_junctions": missing_element_j,
                }
